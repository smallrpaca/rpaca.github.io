...
const ChatBar = inject('states')(
    observer(
        class ChatBar extends Component {
            ...
            // 메세지 내용 변경
            @action
            handleChange(e) { // 흠.. 굳이? 메세지가 바뀐다는걸 명확하게 전달하기위해
            ...
            @action
            send_ChatSend = flow(function * () { // mobx 비동기 처리
            ...
            ReStart() {
                ...
                this.socket.emit('RoomExit', { // 해당 룸값을 서버에 보내 방 나가기
                    nickname: user.nickname,
                    roomName: this.props.roomName
                })

                this.socket.emit('start', { // 시작한다고 서버 요청
                    id: user.id,
                    nickname: user.nickname,
                    gender: user.gender,
