...
const MeetSelect = inject('states')(
    observer(
        class MeetSelect extends Component {
            ...
            @action
            send = (e) => { // Text 버튼을 눌렀을 때 
                const { user } = this.props.states;
        
                if(user.nickname === null) { // 닉네임이 없다면..
                    alert('Please enter a nickname.');
                    return e.preventDefault();
                } else if(user.gender === null) { // 성별이 없다면..
                    alert('Please enter your gender');
                    return e.preventDefault();
                } else if(user.wishgender === null){ // 원하는 성별이 없다면...
                    alert('Please enter your wishgender');
                    return e.preventDefault();
                } else { // 모든 조건이 충족되면 시작
                    this.socket.emit('start', { // 서버에 user에 관한 정보 전송
                        id: user.id,
            ...
            render() {
                ...
                <div className="MeetSelect">
                    <Link to="/meet/text" // TextChat으로 이동
                        className="MeetText"
