    ...
    observer(
        class MeetText extends Component {
            ...
            componentDidMount() {
                ...
                if( user.id === null) { // 홈페이지를 건너뛰고 들어올 경우를 대비... socket.id 받아둠
                    socket.on('connect', () => {
                        user.id = socket.id
                    });
                    alert('User cannot be verified.'); // 사용자를 확인 할 수 없다는 메세지 보냄
                } else { // meet 페이지를 통해 정상적으로 접속했다면...
            ...
            componentWillUnmount() { // 해당 pathname에서 벗어나게 되면...
                const { user } = this.props.states;

                this.socket.emit('RoomExit', { // 서버에게 나갔다고 요청을 보냄.
