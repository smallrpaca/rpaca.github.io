...
io.on('connection', (socket) => {
    ...
    function JoinRoom (RoomName) {
        ...
        socket.join(RoomName, () => { // 실행 함수
            if(rooms[RoomName].length === 2){ // room에 2명이 있다면 해당 room에 메세지 전달.
            ...
            } else if(rooms[RoomName].length === 1){ // 1명이 있다면 기다려달라고 메세지 남김.
                io.sockets.in(RoomName).emit('ChatSend', {
                    nickname: 'Manager',
        ...
        // io.sockets.in(RoomName).emit('userList', {
        //     id: userId,
    ...
    // socket.on('set_userList', (data) => {
    //     if(data.id === socket.id) {
    //         List.push(data.nickname);
    //         console.log(List);
    //     }
